...
public class DialougeTrigger : MonoBehaviour
{
    ...
    [Header("Ch? D?n")]
    [Tooltip("Bi?u tu?ng mui t�n ch? d?n tr�n th? gi?i")]
    public GameObject indicatorPrefab;
    [Tooltip("V? tr� c? d?nh c?a bi?u tu?ng (tuong d?i v?i GameObject n�y)")]
    public Vector3 indicatorOffset = new Vector3(0, 1.5f, 0);
    [Tooltip("T?c d? nh?p nh?y c?a bi?u tu?ng")]
    public float bobSpeed = 2f;
    [Tooltip("�? cao nh?p nh?y c?a bi?u tu?ng")]
    public float bobHeight = 0.2f;
    [Header("Hi?u ?ng xoay")]
    [Tooltip("T?c d? quay c?a bi?u tu?ng (d?/gi�y)")]
    public float rotationSpeed = 45f;
    [Tooltip("C� xoay li�n t?c kh�ng")]
    public bool continuousRotation = true;
    [Tooltip("Tr?c xoay (X, Y, Z ho?c Custom)")]
    public RotationAxis rotationAxis = RotationAxis.Z;
    [Tooltip("G�c xoay t?i da (n?u kh�ng xoay li�n t?c)")]
    public float maxRotationAngle = 45f;
    [Tooltip("Vector hu?ng quay t�y ch?nh (khi ch?n Custom)")]
    public Vector3 customRotationAxis = new Vector3(0, 0, 1);
    [Tooltip("G�c xoay ban d?u (Euler)")]
    public Vector3 initialEulerAngles = Vector3.zero;
    [Header("C�i d?t kh�c")]
    [Tooltip("Hi?n th? bi?u tu?ng c�ng l�c v?i dialogue")]
    ...
    [Tooltip("N?u true, dialogue ch? trigger m?t l?n v� tr?ng th�i s? du?c luu gi?a c�c scene")]
    public bool persistAcrossScenes = false;
    [Tooltip("ID duy nh?t cho dialogue n�y, c?n thi?t khi persistAcrossScenes = true")]
    public string dialogueId;
    [Tooltip("Event g?i khi dialogue b?t d?u")]
    public UnityEvent onDialogueStart;
    [Tooltip("Event g?i khi dialogue k?t th�c")]
    ...
    private void Awake()
    {
        if (persistAcrossScenes)
        {
            if (string.IsNullOrEmpty(dialogueId))
            {
                ...
                Debug.LogWarning($"DialogueId kh�ng du?c cung c?p cho {gameObject.name}. T? d?ng t?o ID: {dialogueId}");
            }
